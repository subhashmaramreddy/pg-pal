...
const testData = {
  ...
  rooms: [
    { name: 'Room 101', capacity: 2, pg_type: 'boys', rent: 5000 },
    { name: 'Room 102', capacity: 2, pg_type: 'boys', rent: 5000 },
    { name: 'Room 103', capacity: 3, pg_type: 'boys', rent: 7500 },
    { name: 'Room 201', capacity: 2, pg_type: 'girls', rent: 5500 },
    { name: 'Room 202', capacity: 2, pg_type: 'girls', rent: 5500 },
  ]
};
...
async function registerAdmin() {
  try {
    ...
    const response = await axios.post(`${API_URL}/auth/admin/register`, testData.admin);
...
async function verifyDatabase() {
  try {
    ...
    // This would require a dedicated endpoint - for now we'll skip
    console.log('✓ Database connection verified from backend startup');
  } catch (error) {
